...
def run_intervention_simulation(
    ...
    full_graphs_30min: Optional[list[Data]] = None,
    device_str: str = "auto",
    verbose: bool = True,
) -> dict:
    """
    ...
    full_graphs_30min：如果有傳（謠言+非謠言的全部 thread），改用
    pheme_graphsage.run_loeo_pretrain_finetune()——先在全部資料上
    打底（讓 GNN 學會怎麼讀懂傳播樹的一般規律），再用 graphs_30min
    （謠言子集）微調。不傳的話（預設 None）維持原本行為，直接用
    run_loeo() 在 graphs_30min 上從頭訓練，不分打底/微調兩階段。
    ...
    import time
    import pandas as pd
    from pheme_graphsage import run_loeo, run_loeo_pretrain_finetune
    ...
    if full_graphs_30min is not None:
        if verbose:
            print("\n先跑 LOEO 訓練（打底+微調兩階段，"
                  "pheme_graphsage.run_loeo_pretrain_finetune），拿每個 fold 的模型...")
        train_results, models = run_loeo_pretrain_finetune(
            rumour_graphs=graphs_30min, full_graphs=full_graphs_30min,
            return_models=True, device_str=device_str,
...
if __name__ == "__main__":
    ...
    # 設環境變數 USE_PRETRAIN_FINETUNE=1 才會啟用打底+微調兩階段訓練，
    # 預設關閉，維持跟之前一樣直接在謠言子集上從頭訓練的行為，不會
    # 因為這次改動悄悄改變你原本已經在用的流程。
    USE_PRETRAIN_FINETUNE = os.getenv("USE_PRETRAIN_FINETUNE", "1") == "1"
    FULL_GRAPHS_30MIN = os.getenv(
        "FULL_GRAPHS_30MIN", "pheme_graphs_roberta_30min_allthreads_v2.pt"
    )

    print("載入圖資料集...")
    graphs_30min = torch.load(GRAPHS_30MIN, weights_only=False)
    print(f"30min（謠言子集）：{len(graphs_30min)} 張圖")

    full_graphs_30min = None
    if USE_PRETRAIN_FINETUNE:
        full_graphs_30min = torch.load(FULL_GRAPHS_30MIN, weights_only=False)
        print(f"30min（全部 thread，打底用）：{len(full_graphs_30min)} 張圖")
    ...
    results = run_intervention_simulation(
        ...
        k_nodes=K_INTERVENE,
        full_graphs_30min=full_graphs_30min,
    )