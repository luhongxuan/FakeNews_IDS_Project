...
def main() -> None:
    output = OUT_ROOT / (datetime.now().strftime("%Y%m%d_%H%M%S_%f") + f"_strict30_quiet_two_expert_tier_{RUN_MODE}")
    ...
    record = {
        ...
        "experts": {"source": "train-only PCA16 of source + observed-reply-centroid embeddings", "temporal": "activity_level + temporal_dynamics clean features"},
    ...
    try:
        ...
        for index, graph in enumerate(graphs, 1):
            ...
            recency[str(graph.thread_id)] = float(temporal[5]); embeddings[str(graph.thread_id)] = np.concatenate([vectors[root], replies.mean(axis=0) if len(replies) else np.zeros_like(vectors[root])])
