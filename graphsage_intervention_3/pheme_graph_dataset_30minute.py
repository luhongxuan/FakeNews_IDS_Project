...
if __name__ == "__main__":
    ...
    ALSO_BUILD_ALLTHREADS = os.getenv("ALSO_BUILD_ALLTHREADS", "1") == "1"
    ...
    # 另外建一份「全部 thread（謠言+非謠言）」的 30 分鐘版本，給
    # run_loeo_pretrain_finetune() 的打底階段用——這個版本用自己的
    # scaler（對全部 thread 的數值分布 fit，不是謠言子集的），
    # 理由跟上面完全一樣：scaler 要對應實際的訓練母體。
    if ALSO_BUILD_ALLTHREADS and ONLY_RUMOUR:
        print(f"\n{'='*55}")
        print("另外建立「全部 thread」版本（給打底階段用）...")
        allthreads_scaler = fit_scaler(feat_df)
        allthreads_graphs = build_graph_dataset(
            feat_df, reply_df, allthreads_scaler, window_sec=1800, only_rumour=False
        )
        save_dataset(allthreads_graphs, "pheme_graphs_roberta_30min_allthreads_v2.pt")
        g = allthreads_graphs[0]
        print(f"\n[30min-allthreads] 第一張圖：x={g.x.shape}, edges={g.edge_index.shape[1]}, "
              f"y={g.y}, node_y_mean={g.node_y.mean():.4f}")